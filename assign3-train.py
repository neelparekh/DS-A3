...
trainset = datasets.ImageFolder(root=os.path.join(rd,'train'), transform = transforms.Compose(
	   [transforms.RandomSizedCrop(224),
		transforms.RandomHorizontalFlip(),
		transforms.ToTensor(),
		transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) 
	   ]))
valset   = datasets.ImageFolder(root=os.path.join(rd,'val'), transform = transforms.Compose(
...
batch_size = 5
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=batch_size)  for x in ['train','val']}
